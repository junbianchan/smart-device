...
import co.darma.services.impl.PhysicalServiceImpl;
 ...
 */
public class PhysicalRecordController extends Controller {
    ...
    JavaType type = mapper.getTypeFactory().constructParametricType(List.class, PhysicalForm.class);
    ...
    public Result queryRecords() {
        ...
        try {
            ...
            List<PhysicalRecord> physicalRecordList = physicalService.queryRecordsbyTime(memberId, startTime, endTime);

            Map finalMap = new HashMap<String, List>(1);
            finalMap.put(PhysicalRecord.KEY, physicalRecordList);
    ...
    public Result updateRecords() {
        ...
        try {
            ...
            if (collections != null) {
                ...
                Logger.info("memberId [" + memberId + "] update PhysicalRecord .");


                List<PhysicalForm> formList = mapper.readValue(collectionStr, type);

                ValidatorHandler handler = ValidatorHandler.getValidatorHandlerByName("physical");
                if (handler != null) {
                ...
                physicalService.updatePhysicalRecordList(PhysicalRecord.packageToRecordList(memberId, formList));
                Logger.info("Sitting form list is :" + formList);
